...
function adminLogout(){adminToken='';sessionStorage.removeItem(ADMIN_TOKEN_KEY);showLogin()}
...
function renderSupport(rows){
  const html=rows.length?rows.map(t=>`<tr><td>#${Number(t.id)}</td><td><b>${esc(t.subject)}</b><br><span class="muted">${esc(t.category)} · ${esc(t.role)}</span></td><td>${esc(t.full_name||'')}<br><span class="muted">${esc(t.email||'')}</span></td><td>${t.booking_id?'#'+Number(t.booking_id):'—'}</td><td style="max-width:360px;white-space:normal">${esc(t.message)}</td><td>${pill(t.status)}</td><td><div class="actions"><button class="btn small" onclick="setSupportStatus(${Number(t.id)},'RESOLVED')">Resolve</button><button class="btn secondary small" onclick="setSupportStatus(${Number(t.id)},'OPEN')">Reopen</button></div></td></tr>`).join(''):'<tr><td colspan="7"><div class="empty">No support tickets.</div></td></tr>';
  document.getElementById('adminContent').innerHTML=`<div class="panel"><h2>Support tickets</h2><div class="table-wrap"><table><thead><tr><th>ID</th><th>Issue</th><th>Account</th><th>Booking</th><th>Message</th><th>Status</th><th>Admin action</th></tr></thead><tbody>${html}</tbody></table></div></div>`;
}
