...
// Fresh table name intentionally avoids any partially-created legacy/v2 schema
// on deployed databases. Routes and Admin import this constant, so both sides
// automatically use the same clean storage.
const SKILL_CERTIFICATE_TABLE='worker_skill_certificates_v3';
  ...
  ensurePromise=(async()=>{
    await pool.query(`
      CREATE TABLE IF NOT EXISTS ${SKILL_CERTIFICATE_TABLE} (
        ...
        INDEX idx_skill_cert_v3_status (status,updated_at),
        INDEX idx_skill_cert_v3_service (service_id,status)
