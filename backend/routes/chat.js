...
router.post('/:bookingId',auth,async(req,res,next)=>{try{
 ...
 const receiver=req.user.id===b.user_id?b.worker_user_id:b.user_id;
 ...
 await notify(req.app,receiver,'New message',`New message for Booking #${b.id}.`,'CHAT');
 req.app.get('io')?.to(`user-${receiver}`).emit('booking-message',{bookingId:b.id,message:{id:r.insertId,booking_id:b.id,sender_id:req.user.id,receiver_id:receiver,message,created_at:new Date().toISOString(),sender_name:req.user.fullName}});
 res.status(201).json({success:true,data:{id:r.insertId,booking_id:b.id,sender_id:req.user.id,receiver_id:receiver,message,created_at:new Date().toISOString(),sender_name:req.user.fullName}});
}catch(e){next(e)}});
